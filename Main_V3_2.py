...
def _load_kuz_lib() -> ctypes.CDLL:
    ...
    if sys.platform.startswith("win"):
        ...
        compile_hint = (
            "Compile it with:\n"
            "  gcc -O3 -shared -fPIC -o Kuznechik_Fast.so Kuznechik_Fast.c\n"
            "or:\n"
            "  gcc -O3 -shared -fPIC -o libKuznechik_Fast.so Kuznechik_Fast.c"
        )
...
# ---------------------------------------------------------------------------
# Fast SCAR via compiled C (SCAR_Fast.dll / .so)
# ---------------------------------------------------------------------------
def _load_scar_lib() -> ctypes.CDLL:
    """Locate and load the compiled SCAR shared library."""
    here = pathlib.Path(__file__).parent
    candidates = [
        here / "SCAR_Fast.dll",   # Windows
        here / "SCAR_Fast.so",    # Linux
        here / "libSCAR_Fast.so", # Linux (lib-prefixed)
    ]
    for p in candidates:
        if p.exists():
            lib = ctypes.CDLL(str(p))
    ...
    if sys.platform.startswith("win"):
        compile_hint = (
            "Compile it with:\n"
            "  cl /LD /O2 SCAR_Fast.c /Fe:SCAR_Fast.dll\n"
            "or (MinGW):\n"
            "  gcc -O2 -shared -o SCAR_Fast.dll SCAR_Fast.c"
        )
    else:
        compile_hint = (
            "Compile it with:\n"
            "  gcc -O2 -shared -fPIC -o SCAR_Fast.so SCAR_Fast.c\n"
            "or:\n"
            "  gcc -O2 -shared -fPIC -o libSCAR_Fast.so SCAR_Fast.c"
        )
    raise FileNotFoundError(
        f"SCAR shared library not found next to {pathlib.Path(__file__).name}.\n"
        + compile_hint
    )

def _setup_scar_lib(lib: ctypes.CDLL) -> None:
    """Set argtypes/restype for every exported function."""
    _u8p = ctypes.POINTER(ctypes.c_uint8)
    _u16p = ctypes.POINTER(ctypes.c_uint16)
    _i8p  = ctypes.POINTER(ctypes.c_int8)
    _vp   = ctypes.c_void_p
    lib.scar_key_schedule.argtypes = [_u16p, _u16p, _i8p, ctypes.c_int]
    lib.scar_key_schedule.restype  = _vp
    lib.scar_free_ctx.argtypes     = [_vp]
    lib.scar_free_ctx.restype      = None
    lib.scar_encrypt.argtypes      = [_u8p, ctypes.c_size_t, _vp, _u8p, ctypes.c_size_t]
    lib.scar_encrypt.restype       = ctypes.c_size_t
    lib.scar_decrypt.argtypes      = [_u8p, ctypes.c_size_t, _vp, _u8p, ctypes.c_size_t]
    lib.scar_decrypt.restype       = ctypes.c_size_t

_SCAR_LIB: ctypes.CDLL = _load_scar_lib()
...
def scar_encrypt_bytes(plaintext: bytes, swap_map, shift_key,
    ...
    """Encrypt using the fast C SCAR implementation."""
    swap = (ctypes.c_uint16 * 513)(*swap_map.values())
    vals = (ctypes.c_uint16 * 16)(*shift_key["values"])
    ops  = (ctypes.c_int8 * 16)()
    for i in range(16):
        ops[i] = 1 if shift_key["equations"][i] == "+" else -1

    ctx = _SCAR_LIB.scar_key_schedule(swap, vals, ops, rounds)
    if not ctx:
        raise RuntimeError("scar_key_schedule returned NULL")

    n       = len(plaintext)
    buf_in  = (ctypes.c_uint8 * n)(*plaintext) if n > 0 else (ctypes.c_uint8 * 1)()
    out_buf = (ctypes.c_uint8 * max(n * 3 + 64, 256))()
    enc_len = _SCAR_LIB.scar_encrypt(buf_in, n, ctx, out_buf,
                                     ctypes.sizeof(out_buf))
    _SCAR_LIB.scar_free_ctx(ctx)

    if enc_len == 0:
        raise RuntimeError("scar_encrypt returned 0")
    return bytes(out_buf[:enc_len]).decode("utf-8")


def scar_decrypt_bytes(ciphertext: str, swap_map, shift_key,
                       rounds: int = SCAR_ROUNDS) -> bytes:
    """Decrypt using the fast C SCAR implementation."""
    swap = (ctypes.c_uint16 * 513)(*swap_map.values())
    vals = (ctypes.c_uint16 * 16)(*shift_key["values"])
    ops  = (ctypes.c_int8 * 16)()
    for i in range(16):
    ...
    n        = len(ct_bytes)
    buf_in   = (ctypes.c_uint8 * n)(*ct_bytes)
    out_buf  = (ctypes.c_uint8 * max(n * 2 + 64, 256))()
    dec_len  = _SCAR_LIB.scar_decrypt(buf_in, n, ctx, out_buf,
                                      ctypes.sizeof(out_buf))
    _SCAR_LIB.scar_free_ctx(ctx)

    if dec_len == 0:
        raise RuntimeError("scar_decrypt returned 0")
    return bytes(out_buf[:dec_len])
