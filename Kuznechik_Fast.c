...
static void R_inv(uint8_t x[16]) {
    ...
    memmove(x, x + 1, 15);
    x[15] = t;
    /* compute what the LFSR output was (= what was in x[0]) */
    uint8_t l = 0;
    for (int i = 0; i < 15; i++)
        l ^= gf_mul(x[i], L_COEFF[i]);
    l ^= gf_mul(x[15], L_COEFF[15]);
    x[15] ^= l ^ t;   /* undo: l == t, so x[15] unchanged... */
    /* Actually the correct inverse: given R(a)=b, recover a.
       b = (l_a, a[0], ..., a[14])
       a = (b[1], ..., b[15], l_b) where l_b = nabla(b) but that gives a[0] not a[15].
       
       Let me re-derive: R maps a[0..15] -> (l, a[0], ..., a[14]).
       R_inv maps b[0..15] -> (b[1], ..., b[15], nabla(b[1],...,b[15],0) XOR b[0])
       i.e. shift left, compute nabla of shifted block with 0 appended.
    */
    /* Redo correctly */
    /* already shifted left above into x[0..14]=b[1..15], x[15]=b[0]=t */
    /* now x[15] should be nabla(b[1..15], x[15]) XOR b[0] */
    uint8_t fb = 0;
    /* x[0..14] = b[1..15], x[15]=b[0] */
    /* we want nabla applied to (b[1],...,b[15], y) XOR b[0], where y is unknown
       Actually: a = R_inv(b) means b = R(a), so a[0]=b[1], ..., a[14]=b[15], a[15]=?
       nabla(a) = b[0], and nabla(a) = sum L_COEFF[i]*a[i]
       = sum_{i=0}^{14} L_COEFF[i]*b[i+1] + L_COEFF[15]*a[15]
       => a[15] = (b[0] XOR sum_{i=0}^{14} L_COEFF[i]*b[i+1]) / L_COEFF[15]
       Since L_COEFF[15]=1, a[15] = b[0] XOR sum_{i=0}^{14} L_COEFF[i]*b[i+1]
    */
    /* x[0..14] = b[1..15], t = b[0] */
    fb = 0;
    for (int i = 0; i < 15; i++)
...
KUZ_API void kuz_ctr_crypt(
    ...
    uint64_t ctr_hi = 0, ctr_lo = 0;
    for (int i = 0; i < 8; i++)  ctr_hi = (ctr_hi << 8) | iv_be[i];
    for (int i = 8; i < 16; i++) ctr_lo = (ctr_lo << 8) | iv_be[i];

    uint8_t ctr_buf[16], ks_buf[16];
    size_t offset = 0;
    while (offset < length) {
        uint64_t h = ctr_hi, l = ctr_lo;
        for (int i = 7; i >= 0; i--) { ctr_buf[i]   = h & 0xFF; h >>= 8; }
        for (int i = 7; i >= 0; i--) { ctr_buf[8+i] = l & 0xFF; l >>= 8; }
